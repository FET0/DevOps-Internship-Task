...
class ContainerConnection {
    constructor(isDockerRequired = true) {
    ...
    open(hostEndpoint, authenticationToken, multipleLoginSupported, doNotAddAuthToConfig) {
    ...
    getQualifiedImageNameIfRequired(imageName) {
        if (!imageUtils.hasRegistryComponent(imageName)) {
            imageName = this.getQualifiedImageName(imageName);
        }
        return imageName;
    }
    getQualifiedImageName(repository, enforceDockerNamingConvention) {
    ...
    getQualifiedImageNamesFromConfig(repository, enforceDockerNamingConvention) {
        let imageNames = [];
        if (repository) {
    ...
    close(multipleLoginSupported, command) {
    ...
    removeConfigDirAndUnsetEnvVariable() {
    ...
    openRegistryEndpoint(authenticationToken, multipleLoginSupported, doNotAddAuthToConfig) {
    ...
    getExistingDockerConfigFilePath() {
    ...
    getDockerConfigJson(configurationFilePath) {
        let configJson;
    ...
    writeDockerConfigJson(dockerConfigContent, configurationFilePath) {
    ...
    getRegistryUrlsFromDockerConfig() {
        let regUrls = [];
        let existingConfigurationFile = this.getExistingDockerConfigFilePath();
        if (existingConfigurationFile) {
            let existingConfigJson = this.getDockerConfigJson(existingConfigurationFile);
    ...
    prefixRegistryIfRequired(registry, repository) {
        let imageName = repository;
        if (registry) {
